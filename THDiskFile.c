...
READ_WRITE_METHODS(double _Complex, ZDouble,
                   double re; double im;int ret = fscanf(dfself->handle, "%lg,%lg", &re, &im); data[i]=re+I*im; if(ret <= 0) break; else nread++,
...
READ_WRITE_METHODS(float _Complex, ZFloat,
                   float re; float im;int ret = fscanf(dfself->handle, "%g,%g", &re, &im); data[i]=re+I*im; if(ret <= 0) break; else nread++,
                   int ret = fprintf(dfself->handle, "%.9g, %.9g", creal(data[i]), cimag(data[i])); if(ret <= 0) break; else nwrite++)
