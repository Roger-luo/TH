...
#define TH_TENSOR_APPLY_CONTIG_PART(TYPE, TENSOR, CODE) \
{ \
  ptrdiff_t TH_TENSOR_size = THPartTensor_(nElement)(TENSOR); \
  PRAGMA(omp parallel if (TH_TENSOR_size > TH_OMP_OVERHEAD_THRESHOLD)) \
  { \
    size_t num_threads = omp_get_num_threads(); \
    size_t tid = omp_get_thread_num(); \
    ...
    ptrdiff_t TH_TENSOR_end = tid == num_threads - 1 ? TH_TENSOR_size : \
      TH_TENSOR_offset + TH_TENSOR_size / num_threads; \
    ptrdiff_t TENSOR##_len = TH_TENSOR_end - TH_TENSOR_offset; \
    TYPE *TENSOR##_data = THPartTensor_(data)(TENSOR) + TH_TENSOR_offset; \
    CODE \
  } \
}
#else
...
#define TH_TENSOR_APPLY_CONTIG_PART(TYPE, TENSOR, CODE) \
{ \
  TYPE *TENSOR##_data = THPartTensor_(data)(TENSOR); \
  ptrdiff_t TENSOR##_len = THPartTensor_(nElement)(TENSOR); \
  CODE \
}
#endif
...
#define TH_TENSOR_APPLY2_CONTIG_PART(TYPE1, TENSOR1, TYPE2, TENSOR2, CODE) \
{ \
  ptrdiff_t TH_TENSOR_size = THPartTensor_(nElement)(TENSOR1); \
  PRAGMA(omp parallel if (TH_TENSOR_size > TH_OMP_OVERHEAD_THRESHOLD)) \
  { \
    size_t num_threads = omp_get_num_threads(); \
    size_t tid = omp_get_thread_num(); \
    ...
    ptrdiff_t TH_TENSOR_end = tid == num_threads - 1 ? TH_TENSOR_size : \
      TH_TENSOR_offset + TH_TENSOR_size / num_threads; \
    ptrdiff_t TENSOR1##_len = TH_TENSOR_end - TH_TENSOR_offset; \
    TYPE1 *TENSOR1##_data = THPartTensor_(data)(TENSOR1) + TH_TENSOR_offset; \
    TYPE2 *TENSOR2##_data = THTensor_(data)(TENSOR2) + TH_TENSOR_offset; \
    CODE \
  } \
}
...
#define TH_TENSOR_APPLY2_CONTIG_PART(TYPE1, TENSOR1, TYPE2, TENSOR2, CODE) \
{ \
  TYPE1 *TENSOR1##_data = THPartTensor_(data)(TENSOR1); \
  TYPE2 *TENSOR2##_data = THTensor_(data)(TENSOR2); \
  ptrdiff_t TENSOR1##_len = THPartTensor_(nElement)(TENSOR1); \
  CODE \
}
#endif
...
{
#if defined(TH_NTYPE_IS_REAL) || defined(TH_NTYPE_IS_COMPLEX) || defined(TH_NTYPE_IS_HALF)
...
void THTensor_(cpow)(THTensor *r_, THTensor *t, THTensor *src)
{
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) && THTensor_(isContiguous)(src) && THTensor_(nElement)(r_) == THTensor_(nElement)(src)) {
    ...
    #pragma omp parallel for if(sz > TH_OMP_OVERHEAD_THRESHOLD) private(i)
    for (i=0; i<sz; i++)
      rp[i] = TH_MATH_NAME(pow)(tp[i], sp[i]);
  } else {
    TH_TENSOR_APPLY3(ntype, r_, ntype, t, ntype, src, *r__data = TH_MATH_NAME(pow)(*t_data, *src_data););
  }
}
...
void THTensor_(tpow)(THTensor *r_, ntype value, THTensor *t)
{
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) && THTensor_(nElement)(r_) == THTensor_(nElement)(t)) {
    ...
    #pragma omp parallel for if(sz > TH_OMP_OVERHEAD_THRESHOLD) private(i)
    for (i=0; i<sz; i++)
      rp[i] = TH_MATH_NAME(pow)(value, tp[i]);
  } else {
    TH_TENSOR_APPLY2(ntype, r_, ntype, t, *r__data = TH_MATH_NAME(pow)(value, *t_data););
  }
}
...
#elif defined (TH_NTYPE_IS_COMPLEX)
  TH_TENSOR_APPLY2(ntype, r_, ntype, t,
    *r__data = *t_data / TH_MATH_NAME(abs)(*t_data););
#else
  TH_TENSOR_APPLY2(ntype, r_, ntype, t,
    if (*t_data > 0) *r__data = 1;
...
#define LAB_IMPLEMENT_BASIC_FUNCTION(NAME, CFUNC)             \
  ...
  {                                                           \
    ...
    if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t)) { \
      TH_TENSOR_APPLY2_CONTIG(ntype, r_, ntype, t, THVector_(NAME)(r__data, t_data, r__len););  \
    } else {  \
...
#define LAB_IMPLEMENT_BASIC_FUNCTION_PART(NAME, CFUNC)             \
  void THTensor_(NAME)(THPartTensor *r_, THTensor *t)                \
  {                                                           \
    THPartTensor_(resizeNd)(r_, t->nDimension, t->size, NULL); \
    if (THPartTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t)) { \
      TH_TENSOR_APPLY2_CONTIG_PART(part, r_, ntype, t, THVector_(NAME)(r__data, t_data, r__len););  \
    } else {  \
...
LAB_IMPLEMENT_BASIC_FUNCTION(sigmoid,THMath_(sigmoid))
LAB_IMPLEMENT_BASIC_FUNCTION(rsqrt,THMath_(rsqrt))
...
// additionaln math functions
LAB_IMPLEMENT_BASIC_FUNCTION(frac,THMath_(frac))
LAB_IMPLEMENT_BASIC_FUNCTION(erfinv,TH_erfinv)
#endif // real only part

// complex only
#ifdef TH_NTYPE_IS_COMPLEX
LAB_IMPLEMENT_BASIC_FUNCTION_PART(abs,TH_MATH_NAME(abs))
LAB_IMPLEMENT_BASIC_FUNCTION_PART(real,TH_MATH_NAME(real))
LAB_IMPLEMENT_BASIC_FUNCTION_PART(imag,TH_MATH_NAME(imag))
LAB_IMPLEMENT_BASIC_FUNCTION_PART(arg,TH_MATH_NAME(arg))
LAB_IMPLEMENT_BASIC_FUNCTION_PART(proj,TH_MATH_NAME(proj))
// c conjugate is the only function
// that does not use c as name prefix
LAB_IMPLEMENT_BASIC_FUNCTION(conj, TH_MATH_NAME(onj))
LAB_IMPLEMENT_BASIC_FUNCTION(log1p,THMath_(log1p))
...
void THTensor_(lerp)(THTensor *r_, THTensor *a, THTensor *b, ntype weight)
{
  ...
  THTensor_(resizeAs)(r_, a);
  TH_TENSOR_APPLY3(ntype, r_, ntype, a, ntype, b, *r__data = THMath_(lerp)(*a_data, *b_data, weight););
}
    ...
    TH_TENSOR_DIM_APPLY2(ntype, t, ntype, r_, dimension,
                         ...
                         for(i = 0; i < t_size; i++) {
                           sum += TH_MATH_NAME(pow)(
                            TH_ABS(t_data[i*t_stride]), value);
                         }
...
accntype THTensor_(normall)(THTensor *tensor, ntype value)
{
  ...
  if(value == 0) {
    ...
    TH_TENSOR_APPLY(ntype, tensor, sum += TH_ABS(*tensor_data););
    return sum;
  } else if(value == 2) {
    TH_TENSOR_APPLY(ntype, tensor, accntype z = *tensor_data; sum += TH_MULC(z, z););
    return sqrt(sum);
  } else {
    TH_TENSOR_APPLY(ntype, tensor, sum += TH_MATH_NAME(pow)(TH_ABS(*tensor_data), value););
...
accntype THTensor_(dist)(THTensor *tensor, THTensor *src, ntype value)
{
  ...
  TH_TENSOR_APPLY2(ntype, tensor, ntype, src,
                   sum += TH_MATH_NAME(pow)(
                     TH_ABS(*tensor_data - *src_data), value););
  return TH_MATH_NAME(pow)(sum, 1.0/value);
}
...
accntype THTensor_(varall)(THTensor *tensor, int biased)
{
  ...
  accntype sum = 0;
  TH_TENSOR_APPLY(ntype, tensor, sum += TH_MULC(*tensor_data - mean, *tensor_data - mean););
