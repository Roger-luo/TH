...
ntype* THStorage_(data)(const THStorage *self)
{
  return self->data;
}
...
THStorage* THStorage_(newWithSize1)(ntype data0)
{
...
THStorage* THStorage_(newWithSize2)(ntype data0, ntype data1)
{
...
THStorage* THStorage_(newWithSize3)(ntype data0, ntype data1, ntype data2)
{
...
THStorage* THStorage_(newWithSize4)(ntype data0, ntype data1, ntype data2, ntype data3)
{
...
THStorage* THStorage_(newWithData)(ntype *data, ptrdiff_t size)
{
  return THStorage_(newWithDataAndAllocator)(data, size,
                                             &THDefaultAllocator, NULL);
}

THStorage* THStorage_(newWithDataAndAllocator)(ntype* data, ptrdiff_t size,
                                               THAllocator* allocator,
                                               void* allocatorContext) {
  THStorage *storage = THAlloc(sizeof(THStorage));
  storage->data = data;
...
void THStorage_(fill)(THStorage *storage, ntype value)
{
  ptrdiff_t i;
...
void THStorage_(set)(THStorage *self, ptrdiff_t idx, ntype value)
{
  THArgCheck((idx >= 0) && (idx < self->size), 2, "out of bounds");
  self->data[idx] = value;
}

ntype THStorage_(get)(const THStorage *self, ptrdiff_t idx)
{
  THArgCheck((idx >= 0) && (idx < self->size), 2, "out of bounds");
