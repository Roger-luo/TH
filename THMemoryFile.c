...
READ_WRITE_METHODS(float _Complex, ZFloat,
                   int nByteRead_; float re; float im; int ret = sscanf(mfself->storage->data+mfself->position, "%g,%g%n", &re, &im, &nByteRead_); data[i] = re+I*im; nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
...
READ_WRITE_METHODS(double _Complex, ZDouble,
                   int nByteRead_; double re; double im; int ret = sscanf(mfself->storage->data+mfself->position, "%lg,%lg%n", &re, &im, &nByteRead_); data[i] = re + I*im; nByteRead = nByteRead_; if(ret <= 0) break; else nread++,
