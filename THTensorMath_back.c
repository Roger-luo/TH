...
ntype THTensor_(minall)(THTensor *tensor)
{
  ...
  return THError("minall is not supported for complex type");
#else
  ntype theMin;
  ntype value;
...
{
#if defined(TH_NTYPE_IS_COMPLEX)
  return THError("maxall is not supported for complex type");
#else
  ntype theMax;
  ntype value;
...
{
#if defined(TH_NTYPE_IS_COMPLEX)
  return THError("medianall is not supported for complex type");
#else
  THArgCheck(tensor->nDimension > 0, 1, "tensor must have one dimension");
...
#define LAB_IMPLEMENT_BASIC_FUNCTION_PART(NAME, CFUNC)             \
  ...
  {                                                           \
    THTensor_(resizeAs)(r_, t);                               \
    if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t)) { \
      TH_TENSOR_APPLY2_CONTIG(part, r_, ntype, t, THVector_(NAME)(r__data, t_data, r__len););  \
    } else {  \
...
#ifdef TH_NTYPE_IS_COMPLEX
#define TH_MULC(Z, C) ((Z) * TH_MATH_NAME(onj)(Z))
#else
...
#ifdef TH_NTYPE_IS_COMPLEX
#define TH_ABS(Z) TH_MATH_NAME(abs)(Z)
else
#define TH_ABS(Z) TH_MATH_NAME(fabs)(Z)
#endif
